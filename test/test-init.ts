...
describe('init', () => {
    ...
    it('init should read local package.json', () => {
        ...
        return withFixtures({'package.json': JSON.stringify(originalContents)}, async () => {
            const result = await init.init(OPTIONS_YES);
            assert.strictEqual(result, true);
            const contents = await readJson('./package.json');

            assert.notStrictEqual(contents, originalContents, 'the file should have been modified');
            assert.strictEqual(
                contents.some,
    ...
    it(
        'should install template copy if src directory already exists and contains files other ' +
            'than ts',
        () => {
            const FIXTURES = {
                src: {
    ...
    it(
        'should not install the default template if the source directory already exists and does ' +
            'contain ts files',
        () => {
            const EXISTING = 'src';
