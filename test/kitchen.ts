...
const keep = !!process.env.WSTS_KEEP_TEMPDIRS;
...
describe('🚰 kitchen sink', () => {
  ...
  const wstsPath = path.join('node_modules', '.bin', 'wsts');
  ...
  before(() => {
    ...
    fs.renameSync(tarball, 'wsts.tgz');
    const targetPath = path.resolve(stagingPath, 'wsts.tgz');
    console.log('moving packed tar to ', targetPath);
    fs.moveSync('wsts.tgz', targetPath);
  ...
  it('it should run init', () => {
    const args = [
      '-p',
      path.resolve(stagingPath, 'wsts.tgz'),
      'wsts',
      'init',
      // It's important to use `-n` here because we don't want to overwrite
      // the version of wsts installed, as it will trigger the npm install.
      '-n',
    ];
  ...
  it('should use as a non-locally installed module', () => {
    ...
    const WSTS = path.resolve(stagingPath, 'kitchen/node_modules/.bin/wsts');
    ...
    // Test package.json expects a wsts tarball from ../wsts.tgz.
    fs.copySync(
      path.join(stagingPath, 'wsts.tgz'),
      path.join(tmpDir.name, 'wsts.tgz')
    );
    // It's important to use `-n` here because we don't want to overwrite
    // the version of wsts installed, as it will trigger the npm install.
    spawn.sync(WSTS, ['init', '-n'], opts);

    // The `extends` field must use the local wsts path.
    ...
    assert.deepStrictEqual(
      tsconfig.extends,
      './node_modules/wsts/tsconfig-widesky.json'
    );
  ...
  it('should terminate generated files with newline', () => {
    const WSTS = path.resolve(stagingPath, wstsPath);
    spawn.sync(WSTS, ['init', '-y'], execOpts);
    assert.ok(
      fs
  ...
  // Verify the `wsts clean` command actually removes the output dir
  it('should clean', () => {
