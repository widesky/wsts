...
import {ConfigFile, getTSConfig, isYarnUsed, getPkgManagerCommand} from '../src/util';
...
describe('util', () => {
    it('get should parse the correct tsconfig file', async () => {
        ...
        function fakeReadFilep(configPath: string, encoding: string): Promise<string> {
    ...
    it('should follow dependency chain caused by extends files', async () => {
        ...
        const contents = await getTSConfig(FAKE_DIRECTORY, createFakeReadFilep(myMap));
        assert.deepStrictEqual(contents, combinedConfig);
    });

    it(
        'when a file contains an extends field, the base file is loaded first then overridden by ' +
            'the inherited files',
        async () => {
            const FAKE_CONFIG1 = {files: ['b'], extends: 'FAKE_CONFIG2'};
    ...
    it(
        'when reading a file, all filepaths should be relative to the config file currently ' +
            'being read',
        async () => {
